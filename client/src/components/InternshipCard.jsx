    ...
    <main className='border-1 bg-white text-black border-gray-400 rounded-md  '>
        <div className='flex border-b-1 items-center border-gray-400 p-3'>
            ...
            <div className='flex-7 pl-6'>
                <h1 className='text-2xl max-sm:text-lg  font-semibold '>{title}</h1>
                <p className='text-gray-500 max-sm:text-md'>{organization}</p>
            </div>
            <div className='flex-1'>
                <h1 className='bg-red-300 text-center rounded-sm max-sm:text-sm p-1 '>{type}</h1>
            </div>
        </div>
        <div className='p-4'>
            <ul className='text-xl'>
                <li className='flex gap-2 items-center max-sm:text-sm'><ImLocation2 />{location}</li>
                <li className='flex gap-2 items-center max-sm:text-sm'><FaRegClock />{duration}</li>
                <li className='flex gap-2 items-center max-sm:text-sm'><RiMoneyRupeeCircleLine />{stipend}</li>
            </ul>
        </div>
        <div className='p-4 '>
            <ul className='flex gap-4 flex-wrap'>
             {skills_required.map((el,ind)=>{
                return <li key={ind} className='bg-green-200 px-3 py-1 rounded-sm max-sm:text-sm'>{el}</li>
             })}
            </ul>
        </div>
